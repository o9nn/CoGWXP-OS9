 ...
 *
 * Tests for CogUtil, AtomSpace, PLN, 9P, and CogW7OS kernel using the
 * canonical public header APIs.
 *
...
static int test_cogutil_memory(void) {
    ...
    /* Check memory stats (should not crash) */
    cog_mem_stats_t stats;
    cog_mem_get_stats(&stats);
    TEST_ASSERT(stats.allocation_count >= stats.free_count,
        "Allocation count should be >= free count");
...
static int test_cogutil_task_queue(void) {
    ...
    TEST_ASSERT(task_counter == 5, "All tasks should have executed");
...
static int test_atomspace_nodes(void) {
    ...
    /* Add concept nodes */
    atom_handle_t cat = atomspace_add_node(as, ATOM_TYPE_CONCEPT_NODE, "cat");
    TEST_ASSERT(cat != ATOM_HANDLE_INVALID, "Adding node should succeed");

    atom_handle_t dog = atomspace_add_node(as, ATOM_TYPE_CONCEPT_NODE, "dog");
    ...
    /* Retrieve node */
    atom_handle_t found = atomspace_get_node(as, ATOM_TYPE_CONCEPT_NODE, "cat");
    TEST_ASSERT(found == cat, "Should find existing node");

    /* Check type */
    atom_type_t type = atomspace_get_type(as, cat);
    TEST_ASSERT(type == ATOM_TYPE_CONCEPT_NODE, "Type should be CONCEPT_NODE");

    /* Check name */
    const char* name = atomspace_get_name(as, cat);
    TEST_ASSERT(name != NULL && strcmp(name, "cat") == 0, "Name should match");

    /* Non-existent node */
    found = atomspace_get_node(as, ATOM_TYPE_CONCEPT_NODE, "nonexistent");
...
static int test_atomspace_links(void) {
    ...
    /* Create nodes */
    atom_handle_t cat = atomspace_add_node(as, ATOM_TYPE_CONCEPT_NODE, "cat");
    atom_handle_t animal = atomspace_add_node(as, ATOM_TYPE_CONCEPT_NODE, "animal");

    /* Create inheritance link: cat -> animal */
    atom_handle_t outgoing[] = {cat, animal};
    atom_handle_t link = atomspace_add_link(as, ATOM_TYPE_INHERITANCE_LINK, outgoing, 2);
    TEST_ASSERT(link != ATOM_HANDLE_INVALID, "Adding link should succeed");

    /* Retrieve link */
    atom_handle_t found = atomspace_get_link(as, ATOM_TYPE_INHERITANCE_LINK, outgoing, 2);
    TEST_ASSERT(found == link, "Should find existing link");

    /* Check outgoing set */
    TEST_ASSERT(atomspace_get_arity(as, link) == 2, "Arity should be 2");
    const atom_handle_t* out = atomspace_get_outgoing(as, link);
    TEST_ASSERT(out != NULL, "Getting outgoing should succeed");
    TEST_ASSERT(out[0] == cat, "First outgoing should be cat");
    TEST_ASSERT(out[1] == animal, "Second outgoing should be animal");

    /* Check incoming set */
    size_t count = 0;
    atom_handle_t* in = atomspace_get_incoming(as, cat, &count);
    TEST_ASSERT(count == 1, "Incoming count should be 1");
    TEST_ASSERT(in != NULL && in[0] == link, "Incoming should be the link");
    atomspace_query_results_free(in);
...
static int test_atomspace_truth_values(void) {
    ...
    atom_handle_t node = atomspace_add_node_tv(as, ATOM_TYPE_CONCEPT_NODE, "test", tv);
    TEST_ASSERT(node != ATOM_HANDLE_INVALID, "Adding node with TV should succeed");

    /* Retrieve truth value */
    truth_value_t retrieved = atomspace_get_tv(as, node);
    ...
    atomspace_set_tv(as, node, tv);

    retrieved = atomspace_get_tv(as, node);
    ...
    TEST_ASSERT(merged.strength > 0.6 && merged.strength < 0.8,
        "Merged strength should be between inputs");
...
static int test_atomspace_attention_values(void) {
    ...
    atom_handle_t node = atomspace_add_node(as, ATOM_TYPE_CONCEPT_NODE, "test");

    /* Set attention value */
    attention_value_t av = {.sti = 100, .lti = 50, .vlti = true};
    atomspace_set_av(as, node, av);

    /* Retrieve attention value */
    attention_value_t retrieved = atomspace_get_av(as, node);
    ...
    /* Test stimulation */
    atomspace_stimulate(as, node, 10);

    retrieved = atomspace_get_av(as, node);
...
static int test_pln_init(void) {
    ...
    pln_config_t config = pln_config_default();
    config.max_inference_steps = 10;
    config.min_confidence_threshold = 0.1;

    pln_engine_t pln = pln_engine_create(as, &config);
    TEST_ASSERT(pln != NULL, "PLN engine should not be NULL");
...
static int test_pln_truth_value_operations(void) {
    ...
    TEST_ASSERT(result.confidence >= a.confidence || result.confidence >= b.confidence,
        "Revision should not decrease confidence below both inputs");
...
static int test_pln_deduction(void) {
    ...
    atom_handle_t cat = atomspace_add_node(as, ATOM_TYPE_CONCEPT_NODE, "cat");
    atom_handle_t animal = atomspace_add_node(as, ATOM_TYPE_CONCEPT_NODE, "animal");
    atom_handle_t living = atomspace_add_node(as, ATOM_TYPE_CONCEPT_NODE, "living_thing");

    truth_value_t tv = tv_simple(0.9, 0.8);
    atom_handle_t out1[] = {cat, animal};
    atom_handle_t cat_animal =
        atomspace_add_link_tv(as, ATOM_TYPE_INHERITANCE_LINK, out1, 2, tv);

    atom_handle_t out2[] = {animal, living};
    atom_handle_t animal_living =
        atomspace_add_link_tv(as, ATOM_TYPE_INHERITANCE_LINK, out2, 2, tv);

    /* Apply deduction: should infer cat -> living_thing */
    atom_handle_t premises[] = {cat_animal, animal_living};
    rule_application_t* app = pln_apply_rule(pln, PLN_RULE_DEDUCTION, premises, 2);
    TEST_ASSERT(app != NULL, "Deduction should succeed");
    TEST_ASSERT(app->conclusion != ATOM_HANDLE_INVALID, "Should produce conclusion");
    TEST_ASSERT(app->conclusion_tv.strength > 0, "Conclusion should have positive strength");
...
static int test_cogw7_cognitive_integration(void) {
    ...
    /* Add some atoms */
    atom_handle_t test = atomspace_add_node(as, ATOM_TYPE_CONCEPT_NODE, "TestAtom");
    TEST_ASSERT(test != ATOM_HANDLE_INVALID, "Adding atom should succeed");

    /* Let reasoning run */
    cog_time_sleep_ms(150);
